...
///  - `cmd arg `：命令+单个参数；
/// 返回`arg`。
pub(super) fn cmd_arg(cmd: &'static str) -> impl Parser<&'static str, Output = String, Error = ParserError<'static>> {
    context(
        "Cmd_Arg",
        terminated(
            preceded(
...
pub(crate) fn arg_escaped_bracket(input: &str) -> IResult<&str, String, ParserError<'_>> {
    context(
        "arg_escaped_bracket",
...
pub(crate) fn arg(input: &str) -> IResult<&str, String, ParserError<'_>> {
    fold_many1(
        alt((
